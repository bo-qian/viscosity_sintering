...
# 运行 MOOSE
mpiexec -n $SLURM_NTASKS ./viscosity_sintering-opt -i ./examples/3D/viscosity_sintering_IC_3D.i  --no-color 2>&1 | tee -a "${OUTPUT_DIR}/${TASK_ID}.log"
mpiexec -n $SLURM_NTASKS ./viscosity_sintering-opt -i ./examples/3D/viscosity_sintering_3D.i Outputs/file_base="${OUTPUT_DIR}/${TASK_ID}_VS2D" --no-color 2>&1 | tee -a "${OUTPUT_DIR}/${TASK_ID}.log"
